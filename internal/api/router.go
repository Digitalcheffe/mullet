...
func NewRouter(sqldb *sql.DB, jwtSecret []byte, corsOrigins []string, info ServerInfo, staticDir string, authDisabled bool, registry *plugindata.Registry, sched *scheduler.Scheduler, uploadsDir string) http.Handler {
	...
	adminMux.HandleFunc("GET /api/admin/settings/notifications", handleGetNotificationPreferences(sqldb))
	adminMux.HandleFunc("PUT /api/admin/settings/notifications", handlePutNotificationPreferences(sqldb))
	adminMux.HandleFunc("GET /api/admin/settings/webhook", handleGetWebhookConfig(sqldb))
	adminMux.HandleFunc("PUT /api/admin/settings/webhook", handlePutWebhookConfig(sqldb))
	adminMux.HandleFunc("POST /api/admin/settings/webhook/test", handleTestWebhook(sqldb))

...
type cardRequest struct {
	...
	ThemeOverride         json.RawMessage `json:"theme_override"`
	// HeaderText nil/empty means no header (issue #145); HeaderHalign is
	// only meaningful alongside a non-empty HeaderText, same "unused
	// sibling field" relationship as ThemeOverride's keys. There's no
	// HeaderValign (issue #154) -- a header always renders in its own
